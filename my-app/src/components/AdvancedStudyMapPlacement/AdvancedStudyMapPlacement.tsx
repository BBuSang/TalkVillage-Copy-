...
import styles from './AdvancedStudyMapPlacement.module.css';
...
// Images
import AdvancedStudyMapImage from '../../image/Maps/AdvancedStudyMap.png';
...
const AdvancedStudyMapPlacement: React.FC = () => {
  ...
  const handleStageClick = (stage: string) => {
    ...
    const stageId = `300-${chapter.padStart(2, '0')}${stageNum.padStart(2, '0')}`;  // 300으로 시작하는 고급 스테이지

    navigate('/tsc', {
      state: {
        mode: 'stage',
        identifier: stageId,
        level: 'advanced'  // 난이도를 advanced로 설정
      }
    });
  };
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:9999/api/user/getUserInfo?Difficulty=hard`, {  // Difficulty를 hard로 변경
          method: 'GET',
          credentials: 'include',
        });
  ...
  return (
    <div>
      <div className={styles.mapImage}>
        <CreateCanvasMap imagePath={AdvancedStudyMapImage} />
      </div>
...
export default AdvancedStudyMapPlacement;